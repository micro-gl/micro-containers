...
namespace microc {
    namespace chunker_traits {
        ...
        public:
            ...
            void construct(U* p, Args&&... args) {
                ::new(p, microc_new::blah) U(chunker_traits::forward<Args>(args)...);
            }
    ...
    public:
        ...
        using allocator_type_index = typename allocator_type::template rebind<size_type>::other;
        using container_data_type = container_template_type<value_type, allocator_type_data>;
        using container_index_type = container_template_type<size_type, allocator_type_index>;
    ...
    public:
        ...
        public:
            chunk()=delete;
            chunk(const T* data, size_type size) : _data(data), _size(size) {}
        ...
        void push_back(const allocator_aware_chunker & $chunker) {
            ...
            for (size_type ix = 0; ix < $chunker.size(); ++ix) {
                const auto chunk = $chunker[ix];
                for (size_type jx = 0; jx < chunk.size; ++jx) {
        ...
        chunk chunk_for(size_type i) {
            size_type idx_start = _locations[i];
            size_type idx_end = (i+1)<size() ? _locations[i+1] : _data.size();
            size_type size = idx_end - idx_start;
            type_pointer pointer = &(_data[idx_start]);
            return {pointer, size};
        }
        chunk chunk_for(size_type i) const {
            size_type idx_start = _locations[i];
            size_type idx_end = (i+1)<size() ? _locations[i+1] : _data.size();
            size_type size = idx_end - idx_start;
        ...
        chunk operator[](size_type i) { return chunk_for(i); }
        chunk operator[](size_type i) const { return chunk_for(i); }
        void clear() {
            _locations.clear();
            _data.clear();
            _locations.push_back(0);
        ...
        size_type size() const { return _locations.size(); }
        size_type unchunked_size() const { return _data.size(); }
