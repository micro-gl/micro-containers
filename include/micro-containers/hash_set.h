...
namespace microc {
    ...
    private:
        struct node_t {
            ...
            node_t(value_type && $value) :
                    prev(nullptr), next(nullptr), key(microc::traits::move($value)) {}
            template<class... Args>
            node_t(Args&&... args) : prev(nullptr), next(nullptr),
                                     key(microc::traits::forward<Args>(args)...) {}
            node_t * prev;
            node_t * next;
            Key key;
        };
    ...
    public:
        ...
        hash_set(hash_set && other) noexcept :
                hash_set(microc::traits::move(other), other.get_allocator()) {}
        ~hash_set() { clear(); }
        ...
        pair<iterator, bool> insert(Key && value) {
            ...
            auto * node = _alloc_node.allocate(1);
            ::new (node, microc_new::blah) node_t(microc::traits::move(value));
            node_query q = internal_insert_node(node);
            return pair<iterator, bool>(iterator(q.node, q.bucket_index, this), true);
        }
    private:
    ...
    public:
        ...
        pair<iterator, bool> emplace(Args&&... args) {
            return insert(Key(microc::traits::forward<Args>(args)...));
        }
    };

    template<class Key, class Hash, class Allocator>
    bool operator==(const hash_set<Key, Hash, Allocator>& lhs,
                    const hash_set<Key, Hash, Allocator>& rhs ) {
        if(!(lhs.size()==rhs.size())) return false;
        using size_type = typename hash_set<Key, Hash, Allocator>::size_type;
